...
- 목표 : 보고서 내용 배제 & 상향 보고서가 나온 기업 및 분야를 웹 스크래핑 & 시각화
    ...
    - 수집한 데이터는 `SQL`을 이용하여 저장 & 조회 : 테이블 2개 (`reports`, `companies`)
        - `reports` : `회사명`, `보고서 올라온 날짜`, `작성 증권사`, `레포트 인덱스(pk)`
        - `companies` : `회사명(pk)`, `회사코드`, `카테고리`
        - db 이름과 유저 이름, 비밀 번호는 별도의 `_info.py`에 보관, import해서 사용하는 방식
    - 시각화는 3가지
        1. 보고서 수가 나온 산업들 (`파이차트`)
        2. 각 산업의 기업 별 보고서 수 (`막대차트`)
        3. 각 산업의 기업 & 개월 별 보고서 수 (`히트맵`)

## 준비물
- [MySQL 커뮤니티 버전](https://dev.mysql.com/downloads/mysql/)
- 나머지는 `requirements.txt`에 기록 (`파이썬 버전 3.10.4(가상환경)`)

## 일자별 진행 과정 
- 진행 기간 : `220803 ~ 220811` + `220817`, `220819`

### 220819
1. 웹에서 조회되는 데이터가 없을 때 바로 종료할 수 있도록 함(`DataCollect.ipynb`)
2. `sqlite3_ver` - `sqlite3_ver.ipynb` - sqlite3으로도 데이터프레임 구현 & 판다스로 조회하는 것까지 만들어봤음
...
3. 그래프 수정 - 폰트 크기, 모든 데이터 표시(`yticklabels = True`), 축 제거(`xlabel`), 히트맵 0 데이터 표시 x


### 220810
1. 수집 과정 테이블 - `report`
    - `report index`라는 개념이 생겼다면, id는 필요가 없음(각 레포트는 고유하기 때문에)
    - 그래서 테이블을 다시 뜯어고쳐야 한다 : `id`를 없애고 `report_idx`를 `pk`로 지정함
2. 히트맵 만들기
    - 통계값을 얻어내기 위한 연산 뒤 그 값을 변수로 활용하는 게 문제였는데, `reset_index()`가 걱정을 싹 날려주었다
    - 위에서 조회한 가로막대 그래프와 다르게 정렬하는 과정이 달라서 실제 조회되는 순서도 달랐음
    - 그래서 위에서 `value_counts()`를 했을 때 얻은 인덱스값 자체를 아래 히트맵 조회하는 인덱스에 전달만 해줬더니 잘 정렬됨
3. 두 그래프 동시에 띄우기


### 220809
1. `DataProcessing.ipynb` : URL 구현 후 필요없는 데이터들은 제외(`companies`의 `id , code`, `reports`의 `reports_idx`)
     - + `ipynb`에서 생략되는 URL 다 보이게끔 구현
2. `시각화`
    - 상향 산업 수는 `파이차트`로 구현, 산업 내 기업 별 레포트 수는 `가로 바 차트`로 구현
    - 기업 별 레포트는 기간과 나온 레포트 수로도 볼 필요가 있을 듯 - 구현 필요
...
### 220819
1. `except` 구문 내에서 `sys.exit`를 썼음에도 프로그램이 종료되지 않음
    - `sys.exit()` 또한 Exception을 발생시키는 구문임
    - 따라서 try, except 구문 밖에서 sys.exit()를 쓸 것을 권장함 (exception 발생 시 켜지는 스위치 변수 하나를 만들 것)

2. `sqlite3`의 자료형이 `mysql`이랑 달라서 귀찮겠거니하고 안했는데 정말 사소한 몇 부분을 빼면 다 똑같음.
    - 역시 걱정한 것과 실제는 다른 거시다

### 220810 
1. SQL 테이블 완전 제거 : `DROP TABLE 테이블명`
2. `pandas` - `groupby`로 묶은 데이터들 추려내기
    - 어떤 column 내에 다른 column들이 들어간 형태로 출력이 되서 어떻게 쓰지?라는 문제가 있었다
    - 근데 `reset_index()`로 인덱스를 초기화해주니까 깔끔하게 출력됨
        - 이 `reset_index()`는 `groupby` 결과로 나온 `Series`도 데이터프레임으로 깔끔하게 바꿔줌
3. 데이터 조회 순서 유지하기
    - 한 데이터프레임의 인덱스 순서를 다른 데이터프레임에도 그대로 전달하고 싶다면
        - 해당 데이터프레임의 인덱스 객체(`order`)를 다른 데이터프레임의 `.loc[order]` 로 전달하면 정렬됨
4. `.gitignore`에 추가하기 전에 실수로 원격 저장소에 올린 데이터 삭제하기
    - 파일 : `git rm --cached 파일명`
    - 디렉토리 : `git rm --cached -r 디렉토리명`

### 220809
1. 시각화 툴 다루기 : `seaborn`, `matplotlib.pyplot`
    - 그래프 위에 숫자 표시(`seaborn`) : `플롯.bar_label(플롯.containers[0])`
    - `plt.subplots` 이후에 반복문 돌리는데 마지막 그래프에만 출력됨
        - 더 웃긴건 제목 표시는 다 잘되는데 그래프 내용물만 빠짐 
        - `plot` 내부에 `ax` 파라미터를 넣으니까 해결됨 (이라고 추측함)
        - 간격 조정 : `constrained_layout = True` 키면 됨
2. 판다스 다루기
    - 기존 데이터 프레임에서 일부 칼럼들만으로 새로운 데이터 프레임 만들기
        - `new = old[['a', 'b', 'c']]`
    - 멀티인덱스 -> 데이터프레임 : `to_frame`
